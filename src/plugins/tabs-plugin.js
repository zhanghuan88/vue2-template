  ...
  install(Vue) {
    Vue.prototype.$tabs = {
      closeTab(path = router.history.current.fullPath) {
        ...
        if (isEmpty(newTabs)) {
          router.push("/").then();
        } else if (tabs[currentIndex].path === currentTabPath) {
          router
            .push(currentIndex + 1 < tabs.length ? tabs[currentIndex + 1].path : tabs[currentIndex - 1].path)
            .then();
        }
